...
public class HackathonV1Should {

  private static final Logger log = LoggerFactory.getLogger(HackathonV1Should.class.getName());
  ...
  @Parameters({"browser", "width", "height", "device"})
  @Test()
  public void display_logo_across_devices_correctly(
      @Optional("chrome") String browser,
      @Optional("1200") int width,
      @Optional("700") int height,
      @Optional("Laptop") String device) {
    ...
    assertThat(
        hackathonReporter.checkDomIds(
            1,
            "logo is displayed",
            logo.getApplifashionLogo(),
            browser,
            width,
            height,
            device,
            driver.findElement(By.id(logo.getApplifashionLogo())).isDisplayed()));
  }

  @Parameters({"browser", "width", "height", "device"})
  @Test()
  public void display_navigation_items_across_devices_correctly(
      @Optional("chrome") String browser,
      @Optional("1200") int width,
      @Optional("700") int height,
      @Optional("Laptop") String device) {
    ...
    SoftAssertions softAssertions = new SoftAssertions();
    hackathonReporter = new HackathonReporter();
    for (Map.Entry<String, String> navigationLink : navigation.getAllNavigationLinks().entrySet()) {
      softAssertions.assertThat(
          hackathonReporter.checkDomIds(
              1,
